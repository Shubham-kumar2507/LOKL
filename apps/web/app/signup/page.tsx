...
export default function SignupPage() {
  ...
  const getPasswordStrength = () => {
    ...
    return {
      width: widths[score],
      color: colors[score],
      label: password.length ? labels[score] : ''
    };
  };
          ...
          <div ref={cardRef} style={styleSheet.card3d}>
            <div style={styleSheet.card}>
              ...
              <div style={styleSheet.header}>
                ...
                <h1 style={styleSheet.title}>Create account</h1>
                <p style={styleSheet.subtitle}>Start your journey with us today</p>
              </div>

              {/* Tabs */}
              <div style={styleSheet.tabs}>
                <button style={styleSheet.tab} onClick={() => router.push("/login")}>Sign In</button>
                <button style={{...styleSheet.tab, ...styleSheet.tabActive}} disabled>Create Account</button>
              </div>

              {/* Form */}
              <form onSubmit={handleSignup}>
                <div style={styleSheet.inputGroup}>
                  <label style={styleSheet.label}>Username</label>
                  <input
                    type="text"
                    placeholder="LOKLGuru"
                    value={username}
                  ...
                  <span style={styleSheet.inputIcon}>
                    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                  </span>
                </div>
                ...
                <div style={styleSheet.inputGroup}>
                  ...
                  <div style={styleSheet.strengthBar}>
                    <div style={{...styleSheet.strengthFill, width: strength.width, background: strength.color}}></div>
                  </div>
                  <div style={{...styleSheet.strengthLabel, color: strength.color}}>{strength.label}</div>
                </div>

                {error && <div style={{ color: '#ef4444', fontSize: '13px', marginBottom: '14px', marginTop: '-6px' }}>{error}</div>}

                <button type="submit" disabled={isSubmitting || !username || !email || !password} style={styleSheet.btnPrimary}>
                  {isSubmitting ? "Creating Account..." : "Create Account"}
                </button>
                
                <p style={styleSheet.terms}>
                  By creating an account you agree to our <a href="#" style={styleSheet.termsLink}>Terms of Service</a> and <a href="#" style={styleSheet.termsLink}>Privacy Policy</a>
                </p>
...
const styleSheet: { [key: string]: React.CSSProperties } = {
  body: {
    ...
    overflow: "hidden",
    color: "#e2eaf8",
    position: "relative" // required for absolute position canvas
  },
  canvas: {
    position: "fixed",
    inset: 0,
  ...
  termsLink: {
    color: "#00d4ff", opacity: 0.75, textDecoration: "none"
  },
  divider: {
